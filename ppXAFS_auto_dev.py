...
from dialog_autoLoad import Ui_Dialog as Ui_Dialog_autoLoad
if sys.platform == 'win32':
    from UI_ppXAFS_win import Ui_MainWindow
    from plot_RawBk_win import Ui_Dialog
    from dialog_autoLoad_win import Ui_Dialog as Ui_Dialog_autoLoad
...
class MainWindow(QtGui.QMainWindow):
    def __init__(self, parent=None):
        ...
        self.dialog_grid.addWidget(self.canvas_dialog,0,0)
        navibar_dialog = NavigationToolbar(self.canvas_dialog, self.dialog.widget)
        self.dialog_grid.addWidget(self.canvas_dialog,0,0)
        self.dialog_grid.addWidget(navibar_dialog)

        self.autoLoad = Ui_Dialog_autoLoad()
        self.dil_auotLoad = QtGui.QDialog()
        ...
        def check_state():
            if os.path.isdir(self.autoLoad.tB_DatDir.toPlainText()) and \
                ...
                self.autoLoad.lineEdit.text() != "" and self.autoLoad.lineEdit_2.text() != "":
        ...
        self.scroll_layout2 = QtGui.QVBoxLayout()
        ...
        self.scroll_layout_exafs = QtGui.QVBoxLayout()
    ...
    def extract_data(self):
        ...
        if len(params.cbs) != 0:
            ...
            if self.u.actionBoost.isChecked():
                pool = multip.Pool()
                M = pool.map(sub_Extract_data,checkedCB_objN)
                energy = M[0][0]
                sumI0 = np.zeros(len(energy))
                sumI1 = np.zeros(len(energy))
                sumI2 = np.zeros(len(energy))
                for i in range(0,len(checkedCB_objN)):
                    if len(M[i][0]) == len(energy):
                        sumI0 += M[i][1]
                        sumI1 += M[i][2]
                        sumI2 += M[i][3]
                        each_ut_star.append(np.array(M[i][2])/np.array(M[i][1]))
                        each_ut.append(np.array(M[i][3]) / np.array(M[i][1]))
                    else:
                        checkBox = self.findChild(QtGui.QCheckBox,checkedCB_objN[i])
    ...
    def readDecay(self,button):
        ...
        csv_buffer = StringIO.StringIO(txt)
        f = open('Text.txt','w')
        ...
        if len(t_)%2 == 0:
            ...
            I1 = df['I1'].values[:N] + df['I1'].values[N:len(t_)][::-1]
            I2 = df['I2'].values[:N] + df['I2'].values[N:len(t_)][::-1]
            mt_star = (df['ut'].values[:N] + df['ut'].values[N:len(t_)][::-1])/2
            mt = (df['ut2'].values[:len(t_)/2] + df['ut2'].values[len(t_)/2:len(t_)][::-1])/2
        elif len(t_)%2 == 1:
            N = (len(t_)-1)/2 #len(t_) = 2*N + 1
            I0 = df['I0'].values[:N] + df['I0'].values[N+1:len(t_)][::-1]
            I1 = df['I1'].values[:N] + df['I1'].values[N+1:len(t_)][::-1]
            I2 = df['I2'].values[:N] + df['I2'].values[N+1:len(t_)][::-1]
            mt_star = (df['ut'].values[:N] + df['ut'].values[N+1:][::-1])/2
            mt = (df['ut2'].values[:N] + df['ut2'].values[N+1:][::-1])/2
    ...
    def doAction(self):
        self.autoLoad.progressBar.setValue(0)
        self.common= self.autoLoad.lineEdit.text()
